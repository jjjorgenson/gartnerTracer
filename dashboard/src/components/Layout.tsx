  ...
  return (
    <div className="min-h-screen text-[var(--color-text)]">
      ...
      <div className="lg:pl-56">
        {banner && (
          <div className="sticky top-0 z-40 flex items-center justify-center gap-2 border-b border-[var(--color-border)] bg-[var(--color-success)]/15 px-4 py-2 text-sm text-[var(--color-text)]">
            {banner}
          </div>
        )}
        <header className="sticky top-0 z-30 flex h-14 shrink-0 items-center gap-3 border-b border-[var(--color-border)] bg-[var(--color-bg-elevated)]/95 px-4 backdrop-blur">
          ...
          <span className="min-w-0 truncate text-sm text-[var(--color-text-subtle)]">
            Search (placeholder)
          </span>
          <div className="ml-auto flex shrink-0 items-center gap-2">
            {hasApi() && apiError === 'unauthorized' && (
              <a
                href={getLoginUrl()}
                className="rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-card)] px-3 py-1.5 text-sm font-medium text-[var(--color-text)] hover:bg-[var(--color-border)]"
              >
                Log in with GitHub
              </a>
            )}
                ...
                {repos.length > 1 && (
                  <div className="relative" ref={repoDropdownRef}>
                    <button
                      ...
                      className="flex items-center gap-1 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-card)] px-3 py-1.5 text-sm font-medium text-[var(--color-text)] hover:bg-[var(--color-border)]"
                ...
                <div className="relative" ref={userMenuRef}>
                  ...
                  {userMenuOpen && (
                    <div className="absolute right-0 top-full z-50 mt-1 min-w-[160px] rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-elevated)] py-1 shadow-lg">
                      ...
                      <a
                        href={getLogoutUrl()}
                        className="block px-3 py-2 text-left text-sm text-[var(--color-text)] hover:bg-[var(--color-border)]"
                      >
                        Log out
                      </a>
                    </div>
...
function SyncButton() {
  const handleSync = () => {
    window.dispatchEvent(new CustomEvent('tracer-refresh'))
  }
  return (
    <button
      type="button"
...
function ManageReposModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" role="dialog" aria-modal="true" aria-labelledby="manage-repos-title">
      <div className="max-h-[80vh] w-full max-w-md overflow-hidden rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-elevated)] shadow-xl">
        <div className="flex items-center justify-between border-b border-[var(--color-border)] px-4 py-3">
          <h2 id="manage-repos-title" className="text-lg font-semibold text-[var(--color-text)]">Manage Repos</h2>
          <button type="button" onClick={onClose} className="rounded p-1 text-[var(--color-text-muted)] hover:bg-[var(--color-border)] hover:text-[var(--color-text)]" aria-label="Close">×</button>
        </div>
        <div className="max-h-[60vh] overflow-y-auto p-4">
          {repos.length === 0 ? (
            <p className="text-sm text-[var(--color-text-subtle)]">No repos connected. Use Connect Repo to add one.</p>
          ) : (
              ...
              {repos.map((r) => (
                <li key={r.repo} className="flex items-center justify-between gap-2 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-card)] px-3 py-2">
                  ...
                  {confirmRepo === r.repo ? (
                    ...
                    <button type="button" onClick={() => setConfirmRepo(r.repo)} className="shrink-0 text-xs font-medium text-[var(--color-text-muted)] hover:text-[var(--color-danger)]">Disconnect</button>
                  )}
                </li>
              ))}
            </ul>
