            ...
            >
              <div className="flex items-center gap-2">
                ...
                {u.deliveryRef && (u.deliveryRef.startsWith('http') ? (
                  ...
                  <span className="text-sm text-slate-500">{u.deliveryRef}</span>
                ))}
              </div>
  ...
  return (
    <span
      className={`rounded px-2 py-0.5 text-xs font-medium ${
        ok ? 'bg-emerald-900/50 text-emerald-300' : status === 'failed' || status === 'rejected' ? 'bg-red-900/50 text-red-300' : 'bg-[var(--color-border)] text-[var(--color-text-muted)]'
      }`}
      role="status"
      aria-label={status}
    >
